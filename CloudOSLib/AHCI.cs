...
namespace CloudOSLib
{
    ...
    {
        KernelClassDebugger ahciDebugger = new("CloudOS AHCI");
        ...
        /// </summary>
        public void Init(int i, PCIDevice dvc)
        {
            Console.Clear();
            ahciDebugger.Send("_");
            try
            {
                Console.WriteLine("SATA INIT");
    
                ahciDebugger.Send("in start----------");
                ...
                {
                    ahciDebugger.Send($"STR PORT Name: {port.mPortName} | STR Num: {port.mPortNumber} | BlockSize: {port.BlockSize}");
                    if (port.mPortType == PortType.SATA) { mPortName = port.mPortName; mPortNum = port.mPortNumber; };
                    aAddressX = 128;
                    StoragePort.Devices.Add(port);
                }
                StoragePort.Devices.ForEach(device => { ahciDebugger.Send($"(old)FOUND A STORAGE DEVICE !. Type: {GetBlockType(device.Type)} | BlockSize: {device.BlockSize} | BlockCount: {device.BlockCount}"); });
                ahciDebugger.Send($"ADDR: {aAddressX} | PortNum: {mPortNum}");
                PortRegisters prtg = new PortRegisters(aAddressX, mPortNum);
                ahciDebugger.Send($"is Active prtg : {prtg.Active} | port num: {prtg.mPortNumber} | port CMD: {prtg.CMD} | Resrv : {prtg.Reserved}");
                ahciDebugger.Send("BEFORE");
                BlockDevice block = null;
                //try { block = new SATA(prtg); } catch (Exception ex) { ahciDebugger.Send($"!!!! ERR !: {ex.Message}"); }
                ahciDebugger.Send("AFTER");
                ...
                {
                    ahciDebugger.Send($"one is bsize = {satadevice.BlockSize} | bcount = {satadevice.BlockCount} | {GetBlockType(satadevice.Type)}");
                }
            }
        ...
        private void BSOD(Exception exception)
        {
            ...
            Console.Clear();
            ahciDebugger.Send("KERNEL ERROR:");
            ahciDebugger.Send($"{exception.GetType().ToString()}\n");
            ahciDebugger.Send($"Details: {exception.ToString()}\n");
            ahciDebugger.Send($"Message: {exception.Message}\n");
            //uint addV;
            //unsafe { Exception* ptr = &exception; addV = (uint)new IntPtr(ptr).ToInt32(); }

            ahciDebugger.Send($"\nInstruction Counter : {exception.HResult}");
            //ahciDebugger.Send($"Last Instruction : {CI}");
            //ahciDebugger.Send($"Linked to : {exception.HelpLink.ToCharArray()}");
